								...
								.on('result', function(res) {
									res.on('row', function(row) {
											if (row['COUNT(item_name)=0'] == 1) {
												c.query('INSERT INTO \`' + which_group + '_data\` SET circle_id = ?,item_name = ?, item_price = ?,\`' + nickname + '\` = ?', [circle_number, item_name, item_price, '{"bought":0,"target":' + item_copy_number + ',"finish":0,"whobuy":"' + responsibility + '"}'])
													.on('result', function(res) {
														res.on('row', function(row) {
			...
			.on('result', function(res) {
				res.on('row', function(row) {
					...
					.on('end', function(info) {
						...
						// socket.emit('main_csv_submit_client',csvdata[1]); //添加成功,返回circle id
					})
			})
			.on('end', function() {
